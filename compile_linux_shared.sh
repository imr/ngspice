...
# CIDER, XSPICE, and OpenMP are not supported in this minimal build.
...
# (depending on the Linux distribution, and on the calling programs search path).
  ../configure --with-ngshared --prefix=/usr CFLAGS="-g -m64 -O0 -Wall" LDFLAGS="-m64 -g"
else
   cd releasesh
...
# The --prefix (and perhaps --libdir) may be used to determine a different install location
# (depending on the Linux distribution, and on the calling programs search path).
  ../configure --with-ngshared --disable-debug --prefix=/usr CFLAGS="-m64 -O2" LDFLAGS="-m64 -s"
fi
