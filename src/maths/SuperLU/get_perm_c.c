	  ...
	  )
{
	...
	ABORT_SuperLU("SUPERLU_MALLOC fails for t_colptr[]");
    if ( !(t_rowind = (int*) SUPERLU_MALLOC( nz * sizeof(int)) ) )
	ABORT_SuperLU("SUPERLU_MALLOC fails t_rowind[]");
    ...
    *bnz = num_nz;
	...
	ABORT_SuperLU("SUPERLU_MALLOC fails for b_colptr[]");
    if ( *bnz) {
      if ( !(*b_rowind = (int*) SUPERLU_MALLOC( *bnz * sizeof(int)) ) )
	ABORT_SuperLU("SUPERLU_MALLOC fails for b_rowind[]");
    }
...
get_perm_c(int ispec, SuperMatrix *A, int *perm_c)
{
	...
	if ( m != n ) ABORT_SuperLU("Matrix is not square");
	...
	if ( !invp ) ABORT_SuperLU("SUPERLU_MALLOC fails for invp.");
	dhead = (int *) SUPERLU_MALLOC((n+delta)*sizeof(int));
	if ( !dhead ) ABORT_SuperLU("SUPERLU_MALLOC fails for dhead.");
	qsize = (int *) SUPERLU_MALLOC((n+delta)*sizeof(int));
	if ( !qsize ) ABORT_SuperLU("SUPERLU_MALLOC fails for qsize.");
	llist = (int *) SUPERLU_MALLOC(n*sizeof(int));
	if ( !llist ) ABORT_SuperLU("SUPERLU_MALLOC fails for llist.");
	marker = (int *) SUPERLU_MALLOC(n*sizeof(int));
	if ( !marker ) ABORT_SuperLU("SUPERLU_MALLOC fails for marker.");
