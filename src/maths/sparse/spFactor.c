...
static char RCSid[] = "@(#)$Header: /cvsroot/sparse/src/spFactor.c,v 1.3 "
...
int spOrderAndFactor(MatrixPtr Matrix, spREAL RHS[], spREAL RelThreshold,
                     spREAL AbsThreshold, int DiagPivoting) {
    ElementPtr pPivot;
    int Step, Size;
    ...
    if (NOT Matrix->NeedsOrdering) {
        ...
        for (Step = 1; Step <= Size; Step++) {
            ...
            if (!pPivot) {
                fprintf(
                    stderr,
                    "Warning: spfactor.c, 230, Pivot for step = %d not found\n",
                    Step);
         ...
         * required rather than a partial reordering, which occurs during a
         * failure of a fast factorization.
         */
        Step = 1;
...
int spFactor(MatrixPtr Matrix) {
    ...
    for (Step = 2; Step <= Size; Step++) {
        if (Matrix->DoRealDirect[Step]) { /* Update column using direct
                                             addressing scatter-gather. */
            ...
            while (pColumn->Row < Step) /* FIXME: Access to Row results in
...
static int FactorComplexMatrix(MatrixPtr Matrix) {
    ...
    /* Start factorization. */
    for (Step = 2; Step <= Size; Step++) {
        if (Matrix->DoCmplxDirect[Step]) { /* Update column using direct
                                              addressing scatter-gather. */
            ComplexNumber *Dest;
                                           ...
                                           dereference of NULL pointer */
            {
                ...
                while ((pElement = pElement->NextInCol) !=
                       NULL) { /* Cmplx expr: Dest[pElement->Row] -= Mult *
                                  pElement */
                                           ...
                                           dereference of NULL pointer */
            {
                ...
                while ((pElement = pElement->NextInCol) !=
                       NULL) { /* Cmplx expr: *pDest[pElement->Row] -= Mult *
                                  pElement */
...
#if REAL
        DoRealDirect[Step] =
            (Nm[Step] + No[Step] > 3 * Nc[Step] - 2 * Nm[Step]);
#endif
#if spCOMPLEX
...
#if spCOMPLEX
        DoCmplxDirect[Step] =
            (Nm[Step] + No[Step] > 7 * Nc[Step] - 4 * Nm[Step]);
#endif
#undef generic
#endif
...
/* Create Intermediate vectors for use in MatrixSolve. */
    ...
    if (Matrix->Intermediate == NULL) {
        if ((Matrix->Intermediate = SP_MALLOC(RealNumber, 2 * (Size + 1))) ==
            NULL)
            Matrix->Error = spNO_MEMORY;
    }
#else
...
static void MarkowitzProducts(MatrixPtr Matrix, int Step) {
    ...
    for (I = Step; I <= Size; I++) {
        ...
        if ((*pMarkowitzRow > LARGEST_SHORT_INTEGER AND * pMarkowitzCol != 0)
                OR(*pMarkowitzCol > LARGEST_SHORT_INTEGER AND * pMarkowitzRow !=
                   0)) {
...
static ElementPtr SearchForPivot(MatrixPtr Matrix, int Step,
    ...
    if (DiagPivoting) {
         ...
         * diagonal of remaining submatrix with smallest Markowitz product, then
         * check to see if it okay numerically.  If not, QuicklySearchDiagonal
         * fails.
         */
        ...
        /*
         * Quick search of diagonal failed, carefully search diagonal and check
         * each pivot candidate numerically before even tentatively accepting
         * it.
         */
...
static ElementPtr SearchForSingleton(MatrixPtr Matrix, int Step) {
    ...
    while (Singletons-- > 0) {
         ...
         * MarkowitzProduct array.  Search terminates when singleton (Product =
         * 0) is found.  Note that the conditional in the while statement (
        ...
        if ((ChosenPivot = Matrix->Diag[I]) != NULL) {
            ...
            if (Matrix->MarkowitzCol[I] == 0) {
                ...
                if (ChosenPivot == NULL) { /* Reduced column has no elements,
                                              matrix is singular. */
                    break;
                }
                PivotMag = ELEMENT_MAG(ChosenPivot);
                if (PivotMag > Matrix->AbsThreshold AND PivotMag >
                    ...
                    if (Matrix->MarkowitzRow[I] == 0) {
                        ...
                        while ((ChosenPivot != NULL)
                                   AND(ChosenPivot->Col < Step))
                            ChosenPivot = ChosenPivot->NextInRow;
                        if (ChosenPivot ==
                            NULL) { /* Reduced row has no elements, matrix is
                                       singular. */
                            break;
                        }
                        PivotMag = ELEMENT_MAG(ChosenPivot);
                ...
                while ((ChosenPivot != NULL) AND(ChosenPivot->Col < Step))
                    ChosenPivot = ChosenPivot->NextInRow;
                if (ChosenPivot == NULL) { /* Reduced row has no elements,
                                              matrix is singular. */
                    break;
                }
                PivotMag = ELEMENT_MAG(ChosenPivot);
...
static ElementPtr SearchEntireMatrix(MatrixPtr Matrix, int Step) {
    ...
    RealNumber Magnitude, LargestElementMag, Ratio, RatioOfAccepted = 0,
                                                    LargestInCol;
    ...
    for (I = Step; I <= Size; I++) {
        ...
        while (pElement != NULL) {
            /* Check to see if element is the largest encountered so far.  If
               so, record its magnitude and address. */
            ...
            if ((Product <= MinMarkowitzProduct)
                ...
                /* Test to see if element has lowest MarkowitzProduct yet found,
...
static RealNumber FindBiggestInColExclude(MatrixPtr Matrix, ElementPtr pElement,
                                          int Step) {
    int Row;
    int Col;
...
static void ExchangeRowsAndCols(MatrixPtr Matrix, ElementPtr pPivot, int Step) {
    ...
    if (Row == Col) {
        ...
        if (Row != Step) {
            ...
            spcMarkoProd(Matrix->MarkowitzProd[Row], Matrix->MarkowitzRow[Row],
                         Matrix->MarkowitzCol[Row]);

            /* Update singleton count. */
            if ((Matrix->MarkowitzProd[Row] == 0) !=
                (OldMarkowitzProd_Row == 0)) {
        ...
        if (Col != Step) {
            ...
            Matrix->NumberOfInterchangesIsOdd =
                NOT Matrix->NumberOfInterchangesIsOdd;
            spcMarkoProd(Matrix->MarkowitzProd[Col], Matrix->MarkowitzCol[Col],
                         Matrix->MarkowitzRow[Col]);

            /* Update singleton count. */
            if ((Matrix->MarkowitzProd[Col] == 0) !=
                (OldMarkowitzProd_Col == 0)) {
        ...
        if ((Matrix->MarkowitzProd[Step] == 0) !=
            (OldMarkowitzProd_Step == 0)) {
...
static void ExchangeColElements(MatrixPtr Matrix, int Row1, ElementPtr Element1,
    ...
    if (Element1 != NULL) {
        ...
        if (ElementBelowRow1->Row != Row2) {
            ...
            ElementBelowRow2 =
                Element2->NextInCol; /* FIXME: ElementBelowRow2 is never read */
...
static void ExchangeRowElements(MatrixPtr Matrix, int Col1, ElementPtr Element1,
    ...
    if (Element1 != NULL) {
        ...
        if (ElementRightOfCol1->Col != Col2) {
            ...
            ElementRightOfCol2 =
                Element2
                    ->NextInRow; /* FIXME: ElementRightOfCol2 is never read */
...
static void RealRowColElimination(MatrixPtr Matrix, ElementPtr pPivot) {
    ...
    while (pUpper != NULL) {
        ...
        while (pLower != NULL) {
            ...
            /* Test to see if desired element was not found, if not, create
             * fill-in. */
...
static void UpdateMarkowitzNumbers(MatrixPtr Matrix, ElementPtr pPivot) {
    ...
    /* Update Markowitz numbers. */
    for (ColPtr = pPivot->NextInCol; ColPtr != NULL;
    ...
    for (RowPtr = pPivot->NextInRow; RowPtr != NULL;
