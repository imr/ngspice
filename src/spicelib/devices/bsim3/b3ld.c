...
int
BSIM3load (GENmodel * inModel, CKTcircuit * ckt)
{
  ...
  double dVasat_dVg, dVasat_dVb, dVasat_dVd, Va, Va2, dVa_dVd, dVa_dVg,
    dVa_dVb;
  ...
  double Xdep, dXdep_dVb, lt1, dlt1_dVb, ltw, dltw_dVb, Delt_vth,
    dDelt_vth_dVb;
  ...
  ChargeComputationNeeded =
    ((ckt->CKTmode & (MODEAC | MODETRAN | MODEINITSMSIG)) ||
     ((ckt->CKTmode & MODETRANOP) && (ckt->CKTmode & MODEUIC))) ? 1 : 0;
	   ...
	   here = here->BSIM3nextInstance)
	{
	  ...
	  if ((ckt->CKTmode & MODEINITSMSIG))
	      ...
	      if ((ckt->CKTmode & MODEINITPRED))
		{
		  ...
		  *(ckt->CKTstate0 + here->BSIM3vbd) =
		    *(ckt->CKTstate0 +
		      here->BSIM3vbs) - *(ckt->CKTstate0 + here->BSIM3vds);
	      ...
	      if (here->BSIM3mode >= 0)
		{
		  ...
		  cbhat = Ibtot +
		    ...
		    (here->BSIM3gbs -
		     here->BSIM3gbbs) * delvbs -
		    here->BSIM3gbgs * delvgs - here->BSIM3gbds * delvds;
		}
	      else
		{
		  Idtot = here->BSIM3cd - here->BSIM3cbd;
		  cdhat = Idtot - (here->BSIM3gbd -
				   here->BSIM3gmbs) *
		    delvbd + here->BSIM3gm * delvgd - here->BSIM3gds * delvds;
		  Ibtot = here->BSIM3cbs + here->BSIM3cbd - here->BSIM3csub;
		  cbhat = Ibtot +
		    here->BSIM3gbs * delvbs +
		    (here->BSIM3gbd -
		     here->BSIM3gbbs) * delvbd -
		    here->BSIM3gbgs * delvgd + here->BSIM3gbds * delvds;
		}
		...
		if ((fabs (delvbs) <
		     (ckt->CKTreltol *
		      MAX (fabs (vbs),
			   fabs (*(ckt->CKTstate0 + here->BSIM3vbs))) +
		      ckt->CKTvoltTol)))
		  if ((fabs (delvbd) <
		       (ckt->CKTreltol *
			MAX (fabs (vbd),
			     fabs (*(ckt->CKTstate0 + here->BSIM3vbd))) +
			ckt->CKTvoltTol)))
		    if ((fabs (delvgs) <
			 (ckt->CKTreltol *
			  MAX (fabs (vgs),
			       fabs (*(ckt->CKTstate0 + here->BSIM3vgs))) +
			  ckt->CKTvoltTol)))
		      if ((fabs (delvds) <
			   (ckt->CKTreltol *
			    MAX (fabs (vds),
				 fabs (*(ckt->CKTstate0 + here->BSIM3vds))) +
			    ckt->CKTvoltTol)))
			if ((fabs (cdhat - Idtot) <
		...
		{
		  vgs = DEVfetlim (vgs,
				   *(ckt->CKTstate0 + here->BSIM3vgs), von);
		  vds = vgs - vgd;
	  ...
	  if (SourceSatCurrent <= 0.0)
	      ...
	      if (model->BSIM3ijth == 0.0)
		{
		  evbs = exp (vbs / Nvtm);
		  here->BSIM3gbs =
		    SourceSatCurrent * evbs / Nvtm + (ckt->CKTgmin);
		  here->BSIM3cbs =
		    SourceSatCurrent * (evbs -
					1.0) + (ckt->CKTgmin) * vbs;
		  ...
		  if (vbs < here->BSIM3vjsm)
			...
			case BSIM3V322:
			  ...
			  here->BSIM3cbs =
			    ...
			    + T0 * (vbs -
				    here->
				    BSIM3vjsm) + (ckt->CKTgmin) * vbs;

			  break;
			case BSIM3V32:
			default:
			  T0 = (SourceSatCurrent + model->BSIM3ijth) / Nvtm;
			  here->BSIM3gbs = T0 + (ckt->CKTgmin);
			  here->BSIM3cbs =
			    model->
			    BSIM3ijth +
			    (ckt->CKTgmin) * vbs + T0 * (vbs -
							     here->BSIM3vjsm);
			}
	  ...
	  if (DrainSatCurrent <= 0.0)
	      ...
	      if (model->BSIM3ijth == 0.0)
		{
		  evbd = exp (vbd / Nvtm);
		  here->BSIM3gbd =
		    DrainSatCurrent * evbd / Nvtm + (ckt->CKTgmin);
		  here->BSIM3cbd =
		    DrainSatCurrent * (evbd - 1.0) + (ckt->CKTgmin) * vbd;
		}
	      else
		{
		  if (vbd < here->BSIM3vjdm)
			...
			case BSIM3V322:
			  ...
			  here->BSIM3cbd =
			    ...
			    + T0 * (vbd -
				    here->
				    BSIM3vjdm) + (ckt->CKTgmin) * vbd;

			case BSIM3V32:
			default:
			  T0 = (DrainSatCurrent + model->BSIM3ijth) / Nvtm;
			  here->BSIM3gbd = T0 + (ckt->CKTgmin);
			  here->BSIM3cbd =
			    model->BSIM3ijth + (ckt->CKTgmin) * vbd +
	  ...
	  if (Vbseff > 0.0)
	    {
	      ...
	      dPhis_dVb = -T0 * T0;
	      sqrtPhis =
		pParam->BSIM3phis3 / (pParam->BSIM3phi + 0.5 * Vbseff);
	  ...
	  dXdep_dVb =
	    (pParam->BSIM3Xdep0 / pParam->BSIM3sqrtPhi) * dsqrtPhis_dVb;
...
/* Vth Calculation */
	  ...
	  dlt1_dVb =
	    model->BSIM3factor1 * (0.5 / T3 * T1 * dXdep_dVb + T3 * T2);
	  ...
	  T1 = pParam->BSIM3k1ox * (T0 -
				    1.0) *
	    pParam->BSIM3sqrtPhi + (pParam->BSIM3kt1 +
				    pParam->BSIM3kt1l /
				    Leff +
	  ...
	  Vth = model->BSIM3type * pParam->BSIM3vth0 -
	    pParam->BSIM3k1 * pParam->BSIM3sqrtPhi +
	    pParam->BSIM3k1ox * sqrtPhis -
	    pParam->BSIM3k2ox * Vbseff - Delt_vth - T2 +
	    (pParam->BSIM3k3 +
	  ...
	  dVth_dVb =
	    pParam->BSIM3k1ox * dsqrtPhis_dVb -
	    pParam->BSIM3k2ox - dDelt_vth_dVb - dT2_dVb +
	    pParam->BSIM3k3b * tmp2 -
	    pParam->BSIM3etab * Vds *
	    pParam->BSIM3theta0vb0 * T4 + pParam->BSIM3kt2 * TempRatio;
...
/* Poly Gate Si Depletion Effect */
	  ...
	  if ((pParam->BSIM3ngate > 1.e18)
	      && (pParam->BSIM3ngate < 1.e25) && (Vgs > T0))
	    /* added to avoid the problem caused by ngate */
	    {
	      T1 = 1.0e6 * Charge_q * EPSSI *
		pParam->BSIM3ngate / (model->BSIM3cox * model->BSIM3cox);
...
/* Effective Vgst (Vgsteff) Calculation */
	  ...
	  if (VgstNVt > EXP_THRESHOLD)
	  ...
	  else
	    {
	      ...
	      dT1_dVb =
		-dT1_dVg * (dVth_dVb + Vgst / n * dn_dVb) + T1 / n * dn_dVb;
	      dT1_dVd =
		-dT1_dVg * (dVth_dVd + Vgst / n * dn_dVd) + T1 / n * dn_dVd;

	      dT2_dVg =
		-model->BSIM3cox / (Vtm * pParam->BSIM3cdep0) * exp (ExpArg);
	      T2 = 1.0 - T10 * dT2_dVg;
	      dT2_dVd =
		-dT2_dVg * (dVth_dVd -
			    2.0 * Vtm * ExpArg *
			    dn_dVd) + (T2 - 1.0) / n * dn_dVd;
	      dT2_dVb =
		-dT2_dVg * (dVth_dVb -
			    2.0 * Vtm * ExpArg *
			    dn_dVb) + (T2 - 1.0) / n * dn_dVb;
...
/* Calculate Effective Channel Geometry */
	  ...
	  Weff = pParam->BSIM3weff -
	    2.0 * (pParam->BSIM3dwg * Vgsteff + pParam->BSIM3dwb * T9);
	  ...
	  if (T0 >= -0.9)
	    {
	      ...
	      dRds_dVb =
		pParam->BSIM3rds0 * pParam->BSIM3prwb * dsqrtPhis_dVb;
	    }
	  else
	    /* to avoid the discontinuity problem due to prwg and prwb */
	    {
	      ...
	      dRds_dVb =
		pParam->BSIM3rds0 * pParam->BSIM3prwb * dsqrtPhis_dVb * T1;
	    }
	  ...
	  if (model->BSIM3mobMod == 1)
	    {
	      ...
	      dDenomi_dVg =
		(T2 + 2.0 * pParam->BSIM3ub * T3) / model->BSIM3tox;
	      dDenomi_dVd = dDenomi_dVg * 2.0 * dVth_dVd;
	      dDenomi_dVb =
		dDenomi_dVg * 2.0 * dVth_dVb + pParam->BSIM3uc * T3;
	    }
	  else if (model->BSIM3mobMod == 2)
	    {
	      T5 = Vgsteff / model->BSIM3tox *
		(pParam->BSIM3ua +
		 pParam->BSIM3uc * Vbseff +
		 pParam->BSIM3ub * Vgsteff / model->BSIM3tox);
	      dDenomi_dVg =
		(pParam->BSIM3ua +
		 pParam->BSIM3uc * Vbseff +
		 2.0 * pParam->BSIM3ub * Vgsteff /
		 model->BSIM3tox) / model->BSIM3tox;
	      dDenomi_dVd = 0.0;
	  ...
	  else
	    {
	      ...
	      dDenomi_dVg =
		(pParam->BSIM3ua +
		 2.0 * pParam->BSIM3ub * T3) * T2 / model->BSIM3tox;
	      dDenomi_dVd = dDenomi_dVg * 2.0 * dVth_dVd;
	      dDenomi_dVb =
		dDenomi_dVg * 2.0 * dVth_dVb + pParam->BSIM3uc * T4;
	    }
   ...
   caused by a1 and a2 (Lambda) */
	  ...
	  if ((Rds == 0.0) && (Lambda == 1.0))
	  ...
	  else
	    {
	      ...
	      dT0_dVb =
		2.0 * (T8 *
		       (2.0 / Abulk * dAbulk_dVb +
			tmp3) + (1.0 / Lambda - 1.0) * dAbulk_dVb);
	      dT0_dVd = 0.0;
		...
		EsatL * dAbulk_dVg + 3.0 * (T9 + T7 * tmp2 + T6 * dAbulk_dVg);
	      dT1_dVb =
		Abulk * dEsatL_dVb +
		EsatL * dAbulk_dVb + 3.0 * (T6 * dAbulk_dVb + T7 * tmp3);
		...
		+ T6 * (4.0 + 2.0 * Vgst2Vtm * tmp2);
	      ...
	      dT3_dVg =
		(T1 * dT1_dVg - 2.0 * (T0 * dT2_dVg + T2 * dT0_dVg)) / T3;
	      dT3_dVd =
		(T1 * dT1_dVd - 2.0 * (T0 * dT2_dVd + T2 * dT0_dVd)) / T3;
	      dT3_dVb =
		(T1 * dT1_dVb - 2.0 * (T0 * dT2_dVb + T2 * dT0_dVb)) / T3;

	      dVdsat_dVg =
		(dT1_dVg -
		 (T1 * dT1_dVg - dT0_dVg * T2 -
		  T0 * dT2_dVg) / T3 - Vdsat * dT0_dVg) / T0;
	      dVdsat_dVb =
		(dT1_dVb -
		 (T1 * dT1_dVb - dT0_dVb * T2 -
...
/* Calculate VAsat */
	  ...
	  dT0_dVg =
	    dEsatL_dVg + dVdsat_dVg + T7 * (1.0 +
					    tmp2 *
					    Vgsteff) -
	    T8 * (Abulk * dVdsat_dVg -
		  Abulk * Vdsat / Vgst2Vtm + Vdsat * dAbulk_dVg);

	  dT0_dVb =
	    dEsatL_dVb + dVdsat_dVb +
	    T7 * tmp3 * Vgsteff -
	  ...
	  if ((pParam->BSIM3pclm > 0.0) && (diffVds > 1.0e-10))
	    {
	      ...
	      dVACLM_dVg =
		T0 * dT1_dVg * diffVds -
		T9 * dVdseff_dVg + T1 * diffVds * dT0_dVg;
	      dVACLM_dVb =
		(dT0_dVb * T1 + T0 * dT1_dVb) * diffVds - T9 * dVdseff_dVb;
...
/* Calculate VADIBL */
	  if (pParam->BSIM3thetaRout > 0.0)
	      ...
	      if (T7 >= -0.9)
		{
		  ...
		  dVADIBL_dVg *= T3;
		  dVADIBL_dVb =
		    (dVADIBL_dVb - VADIBL * pParam->BSIM3pdiblb) * T3;
		  dVADIBL_dVd *= T3;
		}
	      else
...
/* Calculate VASCBE */
	  if (pParam->BSIM3pscbe2 > 0.0)
	    {
	      if (diffVds >
		  pParam->BSIM3pscbe1 * pParam->BSIM3litl / EXP_THRESHOLD)
		{
...
/* Calculate Ids */
	  ...
	  dIdsa_dVd =
	    T0 * dIdl_dVd + Idl * (1.0 - dVdseff_dVd - T9 * dVa_dVd) / Va;
	  ...
	  Gm = T0 * dIdsa_dVg - Idsa * (dVdseff_dVg +
					T9 * dVASCBE_dVg) / VASCBE;
	  ...
	  if ((model->BSIM3xpart < 0) || (!ChargeComputationNeeded))
	    {
	      ...
	      here->BSIM3cqdb = here->BSIM3cqsb =
		here->BSIM3cqgb = here->BSIM3cqbb = 0.0;
	      here->BSIM3gtau = 0.0;
	      goto finished;
	    }
	      ...
	      else
		{
		  ...
		  if (model->BSIM3xpart > 0.5)
			...
			{	/* saturation region */
			  T1 = Vdsat / 3.0;
			  qgate = CoxWL *
			    (Vgs_eff - Vfb - pParam->BSIM3phi - T1);
			  ...
			  here->BSIM3cbgb =
			    -(here->
			      BSIM3cggb - Two_Third_CoxWL * dVgs_eff_dVg);
			...
			{	/* linear region */
			  ...
			  qgate = CoxWL *
			    (Vgs_eff -
			     Vfb - pParam->BSIM3phi - 0.5 * (Vds - T3));
			  T10 = T4 * T8;
			  qdrn = T4 * T7;
			  ...
			  T5 = T3 / T1;
			  here->BSIM3cggb =
			    CoxWL * (1.0 - T5 * dVdsat_dVg) * dVgs_eff_dVg;
			  T11 = -CoxWL * T5 * dVdsat_dVb;
			  here->BSIM3cgdb = CoxWL * (T2 - 0.5 + 0.5 * T5);
			  here->BSIM3cgsb =
			    -(here->BSIM3cggb + T11 + here->BSIM3cgdb);
			  T6 = 1.0 / Vdsat;
			  dAlphaz_dVg = T6 * (1.0 - Alphaz * dVdsat_dVg);
			  dAlphaz_dVb =
			    -T6 * (dVth_dVb + Alphaz * dVdsat_dVb);
			  T7 = T9 * T7;
			  T8 = T9 * T8;
			  T9 = 2.0 * T4 * (1.0 - 3.0 * T5);
			  here->BSIM3cdgb =
			    (T7 *
			     dAlphaz_dVg - T9 * dVdsat_dVg) * dVgs_eff_dVg;
			  T12 = T7 * dAlphaz_dVb - T9 * dVdsat_dVb;
			  here->BSIM3cddb = T4 * (3.0 - 6.0 * T2 - 3.0 * T5);
			  here->BSIM3cdsb =
			    -(here->BSIM3cdgb + T12 + here->BSIM3cddb);
			  ...
			  here->BSIM3cbgb =
			    -(here->BSIM3cggb + here->BSIM3cdgb + T10);
			  here->BSIM3cbdb =
			    -(here->BSIM3cgdb + here->BSIM3cddb + T12);
			  here->BSIM3cbsb =
			    -(here->BSIM3cgsb + here->BSIM3cdsb + T0);
			  here->BSIM3qinv = -(qgate + qbulk);
			}
		    }
			...
			{	/* linear region  */
			  ...
			  here->BSIM3cgdb = CoxWL * (T2 - 0.5 + 0.5 * T5);
			  here->BSIM3cgsb =
			    -(here->BSIM3cggb + here->BSIM3cgdb + tmp);
			  ...
			  T6 = 8.0 * Vdsat *
			    Vdsat - 6.0 * Vdsat * Vds + 1.2 * Vds * Vds;
			  T8 = T2 / T1;
			  T7 = Vds - T1 - T8 * T6;
			  qdrn = T4 * T7;
			  T7 *= T9;
			  ...
			  tmp1 = T4 * (2.0 -
				       4.0 *
				       tmp *
				       T6 + T8 * (16.0 * Vdsat - 6.0 * Vds));

			  here->BSIM3cdgb =
			    (T7 *
			     dAlphaz_dVg - tmp1 * dVdsat_dVg) * dVgs_eff_dVg;
			  ...
			  here->BSIM3cddb =
			    T4 * (2.0 -
				  (1.0 /
				   ...
				   2.0 *
				   tmp) *
				  T6 + T8 * (6.0 * Vdsat - 2.4 * Vds));
			  here->BSIM3cdsb =
			    -(here->BSIM3cdgb + T10 + here->BSIM3cddb);
			  ...
			  T12 = (-T7 *
				 dAlphaz_dVg -
				 here->
				 BSIM3cdgb - T0 * dVdsat_dVg) * dVgs_eff_dVg;
			  T11 = -T7 * dAlphaz_dVb - T10 - T0 * dVdsat_dVb;
			  T10 = -4.0 * T4 *
			    (T2 - 0.5 + 0.5 * T5) - here->BSIM3cddb;
			  tmp = -(T10 + T11 + T12);

			  here->BSIM3cbgb =
			    -(here->BSIM3cggb + here->BSIM3cdgb + T12);

			  here->BSIM3cbdb = -(here->BSIM3cgdb + here->BSIM3cddb + T10);	/* bug fix */

			  here->BSIM3cbsb =
			    -(here->BSIM3cgsb + here->BSIM3cdsb + tmp);
			  here->BSIM3qinv = -(qgate + qbulk);
			}
		    }
		  else
			...
			{	/* linear region */
			  ...
			  here->BSIM3cdgb =
			    (T0 *
			     dVdsat_dVg - T7 * dAlphaz_dVg) * dVgs_eff_dVg;
			  ...
			  here->BSIM3cbgb =
			    -(here->BSIM3cggb + 2.0 * here->BSIM3cdgb);
			  here->BSIM3cbdb =
			    -(here->BSIM3cgdb + 2.0 * here->BSIM3cddb);
			  here->BSIM3cbsb =
			    -(here->BSIM3cgsb + 2.0 * here->BSIM3cdsb);
			  here->BSIM3qinv = -(qgate + qbulk);
			}
		    }
		}
		...
		* pParam->BSIM3leffCV;
	      ...
	      else
		{
		  ...
		  dVgsteff_dVd =
		    -dVgsteff_dVg * (dVth_dVd +
				     (Vgst -
				      voffcv) /
				     noff *
				     dnoff_dVd) + Vgsteff / noff * dnoff_dVd;
		  dVgsteff_dVb =
		    -dVgsteff_dVg * (dVth_dVb +
				     (Vgst -
				      voffcv) /
				     noff *
				     dnoff_dVb) + Vgsteff / noff * dnoff_dVb;
		  dVgsteff_dVg *= dVgs_eff_dVg;
		}		/* End of VgsteffCV */
	      ...
	      if (model->BSIM3capMod == 1)
		{
		  ...
		  if (Arg1 <= 0.0)
			...
			default:
			  ...
			  Cgb = -CoxWL *
			    (dVfb_dVb + dVbseffCV_dVb + dVgsteff_dVb);
			}
		  ...
		  if (VdsatCV < Vds)
		  ...
		  else
		    {
		      ...
		      dT3_dVd =
			6.0 * T0 * (4.0 * Vgsteff - T0) / T1 / T1 - 0.5;
			...
			((1.0 -
			  AbulkCV) * dT3_dVb +
			 (0.5 * Vds -
			  T3) * dAbulkCV_dVb) + Cbg1 * dVgsteff_dVb;
		      Cbd1 = -CoxWL * (1.0 -
				       AbulkCV) *
			dT3_dVd + Cbg1 * dVgsteff_dVd;
		      Cbg1 *= dVgsteff_dVg;
		      Cbg += Cbg1;
		      Cbb += Cbb1;
		      Cbd += Cbd1;
			...
			{	/* 40/60 Charge petition model */
			  ...
			  Csg = -2.0 * qsrc /
			    ...
			    (Vgsteff *
			     (3.0 *
			      Vgsteff -
			      8.0 * T0 / 3.0) + 2.0 * T0 * T0 / 3.0);
			  Csb = (qsrc / T1 *
				 Vds +
				 T2 * T4 *
				 Vds) * dAbulkCV_dVb + Csg * dVgsteff_dVb;
			  Csd = (qsrc / T1 +
				 T2 * T4) * AbulkCV + Csg * dVgsteff_dVd;
			  Csg *= dVgsteff_dVg;
			}
		      else
	      ...
	      else if (model->BSIM3capMod == 2)
		{
		  ...
		  switch (model->BSIM3intVersion)
		    ...
		    default:
		      dVfbeff_dVb =
			(1.0 - T1 - T2) * dVfb_dVb - T1 * dVbseffCV_dVb;
		    }
		  ...
		  dQsub0_dVb =
		    -T2 * (dVfbeff_dVb + dVbseffCV_dVb + dVgsteff_dVb);
		  ...
		  Cgb1 = CoxWL * (T5 * dVdseffCV_dVb +
				  T6 * dAbulkCV_dVb) + Cgg1 * dVgsteff_dVb;
		  Cgg1 *= dVgsteff_dVg;
		  ...
		  Cbb1 = CoxWL * (T5 * dVdseffCV_dVb +
				  T6 * dAbulkCV_dVb) + Cbg1 * dVgsteff_dVb;
		  Cbg1 *= dVgsteff_dVg;
		  ...
		  if (model->BSIM3xpart > 0.5)
		    {		/* 0/100 Charge petition model */
		      ...
		      Csb = CoxWL * (T5 *
				     dVdseffCV_dVb +
				     T6 * dAbulkCV_dVb) + Csg * dVgsteff_dVb;
		      Csg *= dVgsteff_dVg;
		    }
		  else if (model->BSIM3xpart < 0.5)
		    {		/* 40/60 Charge petition model */
	      ...
	      else if (model->BSIM3capMod == 3)
		{
		  ...
		  dQsub0_dVg =
		    T2 * (dVgs_eff_dVg -
			  dVfbeff_dVg - dVgsteff_dVg) + QovCox * dCoxeff_dVg;
		  dQsub0_dVd = -T2 * dVgsteff_dVd;
		  dQsub0_dVb =
		    -T2 * (dVfbeff_dVb +
			   dVbseffCV_dVb +
			   dVgsteff_dVb) + QovCox * dCoxeff_dVb;
		  ...
		  if (pParam->BSIM3k1ox <= 0.0)
		    ...
		    {
		      Denomi = pParam->BSIM3moin *
			Vtm * pParam->BSIM3k1ox * pParam->BSIM3k1ox;
		  ...
		  Cgd1 = CoxWLcen * T5 * dVdseffCV_dVd +
		    Cgg1 * dVgsteff_dVd + QovCox * dCoxeff_dVd;
		  Cgb1 = CoxWLcen * (T5 *
				     dVdseffCV_dVb +
				     T6 *
				     dAbulkCV_dVb) +
		  ...
		  qbulk = CoxWLcen * T7 * (0.5 *
					   VdseffCV - T0 * VdseffCV / T2);
		  QovCox = qbulk / Coxeff;
		  Cbg1 = CoxWLcen * (T10 + T11 * dVdseffCV_dVg);
		  Cbd1 = CoxWLcen * T11 *
		    dVdseffCV_dVd +
		    Cbg1 * dVgsteff_dVd + QovCox * dCoxeff_dVd;
		  Cbb1 = CoxWLcen * (T11 *
				     dVdseffCV_dVb +
				     T12 *
				     dAbulkCV_dVb) +
		  ...
		  if (model->BSIM3xpart > 0.5)
		    {		/* 0/100 partition */
		      ...
		      T4 = -(0.5 +
			     24.0 * T0 * T0 / T3) * (1.0 - dDeltaPhi_dVg);
		      T5 = T7 * AbulkCV;
		      T6 = T7 * VdseffCV;
		      ...
		      Csd = CoxWLcen * T5 *
			dVdseffCV_dVd +
			Csg * dVgsteff_dVd + QovCox * dCoxeff_dVd;
		      Csb = CoxWLcen * (T5 *
					dVdseffCV_dVb
					+
					T6 *
			...
			T3 * (T1 *
		      ...
		      Csd = Csg * dVgsteff_dVd +
			T6 * dVdseffCV_dVd + QovCox * dCoxeff_dVd;
	...
	finished:
	  ...
	  if (ChargeComputationNeeded)
		...
		case BSIM3V323:
		  ...
		  czbs = model->
		    BSIM3unitAreaTempJctCap * here->BSIM3sourceArea;
		  break;
	      ...
	      if (here->BSIM3drainPerimeter < pParam->BSIM3weff)
		{
		  ...
		  switch (model->BSIM3intVersion)
		    ...
		    case BSIM3V323:
		      ...
		      czbdswg =
			model->
			BSIM3unitLengthGateSidewallTempJctCap
			* here->BSIM3drainPerimeter;
		      break;
		    case BSIM3V322:
		    case BSIM3V32:
	      ...
	      else
		{
		  ...
		  switch (model->BSIM3intVersion)
		    ...
		    case BSIM3V323:
		      ...
		      czbdsw = model->
			BSIM3unitLengthSidewallTempJctCap
			...
			* pParam->BSIM3weff;
		    ...
		    default:
		      ...
		      czbdsw = model->
			BSIM3unitLengthSidewallJctCap
			* (here->BSIM3drainPerimeter - pParam->BSIM3weff);
		      czbdswg =
			model->
			BSIM3unitLengthGateSidewallJctCap * pParam->BSIM3weff;
		    }
	      ...
	      else if (vbs < 0.0)
		{
		  ...
		  if (czbsswg > 0.0)
			...
			sarg = exp (-MJSWG * log (arg));
		      *(ckt->CKTstate0 +
			here->BSIM3qbs) +=
			model->BSIM3PhiBSWG * czbsswg * (1.0 -
							 arg *
							 sarg)
			/ (1.0 - MJSWG);
	      ...
	      else
		{
		  ...
		  T1 = vbs * (czbs * MJ /
			      ...
			      czbssw * MJSW /
			      model->BSIM3PhiBSW +
			      czbsswg * MJSWG / model->BSIM3PhiBSWG);
	      ...
	      else if (vbd < 0.0)
		{
		  ...
		  if (czbdswg > 0.0)
			...
			sarg = exp (-MJSWG * log (arg));
		      *(ckt->CKTstate0 +
			here->BSIM3qbd) +=
			model->BSIM3PhiBSWG * czbdswg * (1.0 -
							 arg *
							 sarg)
			/ (1.0 - MJSWG);
	      ...
	      else
		{
		  ...
		  T1 = vbd * (czbd * MJ /
			      ...
			      czbdsw * MJSW /
			      model->BSIM3PhiBSW +
			      czbdswg * MJSWG / model->BSIM3PhiBSWG);
	...
	line755:
	  ...
	  if (here->BSIM3nqsMod)
		...
		-(here->BSIM3cqgb + here->BSIM3cqdb + here->BSIM3cqsb);
	      ...
	      gtau_diff =
		16.0 * pParam->BSIM3u0temp *
		model->BSIM3vtm / T0 * ScalingFactor;
	  ...
	  if (model->BSIM3capMod == 0)
	      ...
	      if (vgd < 0.0)
		{
		  ...
		  cgdo = pParam->BSIM3cgdo +
		    pParam->BSIM3weffCV * pParam->BSIM3cgdl / T1;
		  qgdo = pParam->BSIM3cgdo * vgd -
		    pParam->BSIM3weffCV * 0.5 *
		    pParam->BSIM3cgdl * pParam->BSIM3ckappa * (T1 - 1.0);
		}
	      else
		{
		  cgdo = pParam->BSIM3cgdo +
		    pParam->BSIM3weffCV * pParam->BSIM3cgdl;
		  qgdo = (pParam->BSIM3weffCV *
			  pParam->BSIM3cgdl + pParam->BSIM3cgdo) * vgd;
		}
	      ...
	      if (vgs < 0.0)
		{
		  ...
		  cgso = pParam->BSIM3cgso +
		    pParam->BSIM3weffCV * pParam->BSIM3cgsl / T1;
		  qgso = pParam->BSIM3cgso * vgs -
		    pParam->BSIM3weffCV * 0.5 *
		...
		{
		  cgso = pParam->BSIM3cgso +
		    pParam->BSIM3weffCV * pParam->BSIM3cgsl;
		  qgso = (pParam->BSIM3weffCV *
			  pParam->BSIM3cgsl + pParam->BSIM3cgso) * vgs;
		}
	    }
	  else
	    {
	      ...
	      cgdo = pParam->BSIM3cgdo + T3 - T3 * (1.0 -
						    1.0 /
						    T4) *
		(0.5 - 0.5 * T0 / T1);
	      qgdo = (pParam->BSIM3cgdo + T3) * vgd -
		T3 * (T2 + 0.5 * pParam->BSIM3ckappa * (T4 - 1.0));
	      ...
	      T4 = sqrt (1.0 - 4.0 * T2 / pParam->BSIM3ckappa);
	      cgso = pParam->BSIM3cgso + T3 - T3 * (1.0 -
						    1.0 /
						    T4) *
		(0.5 - 0.5 * T0 / T1);
	      qgso = (pParam->BSIM3cgso + T3) * vgs -
		T3 * (T2 + 0.5 * pParam->BSIM3ckappa * (T4 - 1.0));
	    }
	  ...
	  if (here->BSIM3mode > 0)
	      ...
	      if (here->BSIM3nqsMod == 0)
		{
		  ...
		  gcsgb = -(here->BSIM3cggb +
			    here->BSIM3cbgb + here->BSIM3cdgb + cgso) * ag0;
		  gcsdb = -(here->BSIM3cgdb +
			    here->BSIM3cbdb + here->BSIM3cddb) * ag0;
		  gcssb = (here->BSIM3capbs + cgso -
			   (here->BSIM3cgsb +
	      ...
	      else
		{
		  ...
		  CoxWL = model->BSIM3cox *
		    pParam->BSIM3weffCV * pParam->BSIM3leffCV;
		  if (fabs (qcheq) <= 1.0e-5 * CoxWL)
		  ...
		  else
		    {
		      ...
		      Cdd = here->BSIM3cddb;
		      Csd = -(here->BSIM3cgdb +
			      here->BSIM3cddb + here->BSIM3cbdb);
		      ddxpart_dVd = (Cdd - dxpart * (Cdd + Csd)) / qcheq;
		      Cdg = here->BSIM3cdgb;
		      Csg = -(here->BSIM3cggb +
			      here->BSIM3cdgb + here->BSIM3cbgb);
		      ddxpart_dVg = (Cdg - dxpart * (Cdg + Csg)) / qcheq;

		      Cds = here->BSIM3cdsb;
		      Css = -(here->BSIM3cgsb +
			      here->BSIM3cdsb + here->BSIM3cbsb);
		      ddxpart_dVs = (Cds - dxpart * (Cds + Css)) / qcheq;

		      ddxpart_dVb =
			-(ddxpart_dVd + ddxpart_dVg + ddxpart_dVs);
		    }
		  sxpart = 1.0 - dxpart;
	      ...
	      if (here->BSIM3nqsMod == 0)
		{
		  ...
		  gcdgb = -(here->BSIM3cggb +
			    here->BSIM3cbgb + here->BSIM3cdgb + cgdo) * ag0;
		  gcddb = (here->BSIM3capbd + cgdo -
			   (here->BSIM3cgsb +
			    here->BSIM3cbsb + here->BSIM3cdsb)) * ag0;
		  gcdsb = -(here->BSIM3cgdb +
			    here->BSIM3cbdb + here->BSIM3cddb) * ag0;
	      ...
	      else
		{
		  ...
		  if (fabs (qcheq) <= 1.0e-5 * CoxWL)
		  ...
		  else
		    {
		      ...
		      dsxpart_dVb =
			-(dsxpart_dVd + dsxpart_dVg + dsxpart_dVs);
		    }
		  dxpart = 1.0 - sxpart;
	...
	line850:
	  ...
	  if (here->BSIM3nqsMod)
	    here->BSIM3gtau =
	      16.0 * pParam->BSIM3u0temp *
	      model->BSIM3vtm /
	      pParam->BSIM3leffCV / pParam->BSIM3leffCV * ScalingFactor;
	  else
	    here->BSIM3gtau = 0.0;
	  ...
	  if (here->BSIM3nqsMod)
	    {
	      ...
	      ceqqd -= dxpart * T0 +
		T1 * (ddxpart_dVg * vgb -
		      ddxpart_dVd * vbd - ddxpart_dVs * vbs);
	      cqdef = *(ckt->CKTstate0 + here->BSIM3cqcdump) - gqdef * qdef;
	      cqcheq = *(ckt->CKTstate0 +
			 here->BSIM3cqcheq) - (gcqgb * vgb -
					       gcqdb * vbd -
					       gcqsb * vbs) + T0;
	    }
	...
	line900:
	  ...
	  (*(ckt->CKTrhs + here->BSIM3dNodePrime) +=
	   m * (ceqbd - cdreq - ceqqd));
	  (*(ckt->CKTrhs + here->BSIM3sNodePrime) +=
	  ...
	  (*(here->BSIM3BbPtr) +=
	   m * (here->BSIM3gbd + here->BSIM3gbs - gcbgb -
		gcbdb - gcbsb - here->BSIM3gbbs));
	  (*(here->BSIM3DPdpPtr) +=
	   m * (here->BSIM3drainConductance + here->BSIM3gds +
		here->BSIM3gbd + RevSum + gcddb +
		dxpart * ggtd + T1 * ddxpart_dVd + gbdpdp));
	  (*(here->BSIM3SPspPtr) +=
	   m * (here->BSIM3sourceConductance + here->BSIM3gds +
		here->BSIM3gbs + FwdSum + gcssb +
		sxpart * ggts + T1 * dsxpart_dVs + gbspsp));
	  ...
	  (*(here->BSIM3DPgPtr) +=
	   m * (Gm + gcdgb + dxpart * ggtg + T1 * ddxpart_dVg + gbdpg));
	  (*(here->BSIM3DPbPtr) -=
	   m * (here->BSIM3gbd - Gmbs + gcdgb + gcddb + gcdsb -
		dxpart * ggtb - T1 * ddxpart_dVb - gbdpb));
	  (*(here->BSIM3DPspPtr) -=
	   m * (here->BSIM3gds + FwdSum - gcdsb -
		dxpart * ggts - T1 * ddxpart_dVs - gbdpsp));
	  (*(here->BSIM3SPgPtr) +=
	   m * (gcsgb - Gm + sxpart * ggtg + T1 * dsxpart_dVg + gbspg));
	  (*(here->BSIM3SPsPtr) -= m * here->BSIM3sourceConductance);
	  (*(here->BSIM3SPbPtr) -=
	   m * (here->BSIM3gbs + Gmbs + gcsgb + gcsdb + gcssb -
		sxpart * ggtb - T1 * dsxpart_dVb - gbspb));
	  (*(here->BSIM3SPdpPtr) -=
	   m * (here->BSIM3gds + RevSum - gcsdb -
