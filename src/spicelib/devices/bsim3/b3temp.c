...
BSIM3temp (GENmodel * inModel, CKTcircuit * ckt)
{
	...
	BSIM3instance *here;
	struct bsim3SizeDependParam *pSizeDependParamKnot, *pLastKnot,
		*pParam;
	double tmp, tmp1, tmp2, tmp3, Eg, Eg0, ni, T0, T1, T2, T3, T4, T5,
		Ldrn, Wdrn;
	double delTemp, Temp, TRatio, Inv_L, Inv_W, Inv_LW, Dw, Dl, Vtm0,
		Tnom;
	...
	for (; model != NULL; model = model->BSIM3nextModel)
	{
		...
		if (Temp != Tnom)
		{
			T0 = Eg0 / Vtm0 - Eg / model->BSIM3vtm +
				model->BSIM3jctTempExponent * log (Temp /
								   Tnom);
			T1 = exp (T0 / model->BSIM3jctEmissionCoeff);
			model->BSIM3jctTempSatCurDensity =
				model->BSIM3jctSatCurDensity * T1;
		     ...
		     here = here->BSIM3nextInstance)
		{
			...
			Size_Not_Found = 1;
			while ((pSizeDependParamKnot != NULL)
			       && Size_Not_Found)
			{
				if ((here->BSIM3l ==
				     pSizeDependParamKnot->Length)
				    && (here->BSIM3w ==
					pSizeDependParamKnot->Width))
				{
					Size_Not_Found = 0;
				...
				else
				{
					...
					pSizeDependParamKnot =
						pSizeDependParamKnot->pNext;
				}
			}

			if (Size_Not_Found)
			{
				pParam = (struct bsim3SizeDependParam *)
					tmalloc (sizeof
						 (struct
						  bsim3SizeDependParam));
				if (pLastKnot == NULL)
				...
				pParam->BSIM3weff =
					here->BSIM3w - 2.0 * pParam->BSIM3dw;
				...
				pParam->BSIM3leffCV =
					here->BSIM3l - 2.0 * pParam->BSIM3dlc;
				...
				pParam->BSIM3weffCV =
					here->BSIM3w - 2.0 * pParam->BSIM3dwc;
				...
				T0 = pParam->BSIM3leffCV *
					pParam->BSIM3leffCV;
				pParam->BSIM3tconst =
					pParam->BSIM3u0temp *
					pParam->BSIM3elm / (model->BSIM3cox *
							    pParam->
							    BSIM3weffCV *
							    pParam->
							    BSIM3leffCV * T0);
				...
				if (!model->BSIM3npeakGiven
				    && model->BSIM3gamma1Given)
				{
					T0 = pParam->BSIM3gamma1 *
						model->BSIM3cox;
					pParam->BSIM3npeak =
						3.021E22 * T0 * T0;
				}
				...
				pParam->BSIM3Xdep0 =
					sqrt (2.0 * EPSSI /
					      (Charge_q * pParam->BSIM3npeak *
					       1.0e6)) * pParam->BSIM3sqrtPhi;
				pParam->BSIM3sqrtXdep0 =
					sqrt (pParam->BSIM3Xdep0);
				pParam->BSIM3litl =
					sqrt (3.0 * pParam->BSIM3xj *
					      model->BSIM3tox);
				pParam->BSIM3vbi =
					Vtm0 * log (1.0e20 *
						    pParam->BSIM3npeak / (ni *
									  ni));
				pParam->BSIM3cdep0 =
					sqrt (Charge_q * EPSSI *
				...
				else
				{
					...
					T0 = pParam->BSIM3gamma1 -
						pParam->BSIM3gamma2;
					T1 = sqrt (pParam->BSIM3phi -
						   pParam->BSIM3vbx) -
						pParam->BSIM3sqrtPhi;
					T2 = sqrt (pParam->BSIM3phi *
						   (pParam->BSIM3phi -
						    pParam->BSIM3vbm)) -
						pParam->BSIM3phi;
					pParam->BSIM3k2 =
				...
				pParam->BSIM3k1ox =
					pParam->BSIM3k1 * model->BSIM3tox /
					model->BSIM3toxm;
				pParam->BSIM3k2ox =
					pParam->BSIM3k2 * model->BSIM3tox /
					model->BSIM3toxm;

				T1 = sqrt (EPSSI / EPSOX * model->BSIM3tox
					   * pParam->BSIM3Xdep0);
				T0 = exp (-0.5 * pParam->BSIM3dsub *
					  pParam->BSIM3leff / T1);
				pParam->BSIM3theta0vb0 = (T0 + 2.0 * T0 * T0);

				T0 = exp (-0.5 * pParam->BSIM3drout *
					  pParam->BSIM3leff / T1);
				T2 = (T0 + 2.0 * T0 * T0);
				pParam->BSIM3thetaRout =
					pParam->BSIM3pdibl1 * T2 +
					pParam->BSIM3pdibl2;
				...
				T0 = sqrt (1.0 +
					   pParam->BSIM3nlx /
					   pParam->BSIM3leff);
				T5 = pParam->BSIM3k1ox * (T0 -
							  1.0) *
					pParam->BSIM3sqrtPhi +
					(pParam->BSIM3kt1 +
					 pParam->BSIM3kt1l /
					 pParam->BSIM3leff) * (TRatio - 1.0);

				tmp3 = model->BSIM3type * pParam->BSIM3vth0
					- T2 - T3 + pParam->BSIM3k3 * T4 + T5;
				pParam->BSIM3vfbzb =
					tmp3 - pParam->BSIM3phi -
					pParam->BSIM3k1 *
					pParam->BSIM3sqrtPhi;
				/* End of vfbzb */
			}
