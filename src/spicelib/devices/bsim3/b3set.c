...
/* Default value Processing for BSIM3 MOSFET Models */
		...
		if (!model->BSIM3u0Given)
			model->BSIM3u0 =
				(model->BSIM3type == NMOS) ? 0.067 : 0.025;
		...
		{
			model->BSIM3cgbo =
				2.0 * model->BSIM3dwc * model->BSIM3cox;
		}
		     ...
		     here = here->BSIM3nextInstance)
		{
			...
			{
				error = CKTmkVolt (ckt, &tmp, here->BSIM3name,
						   "drain");
				if (error)
					return (error);
