...
do { if((here->ptr = SMPmakeElt(matrix, here->first, here->second)) == NULL){\
    ...
    for (; model != NULL; model = BSIM3SIMDnextModel(model))
    {
    	...
    	first = BSIM3SIMDinstances(model);
    	while((idx<InstCount) && (first!=NULL))
    	{
    		ngroups++;
    		group = TMALLOC(BSIM3group, 1);
    		...
    		for (; here != NULL; here = BSIM3SIMDnextInstance(here))
       		{
       			if((here->BSIM3nqsMod == group->InstArray[0]->BSIM3nqsMod)
			&& (here->BSIM3w == group->InstArray[0]->BSIM3w)
			&& (here->BSIM3l == group->InstArray[0]->BSIM3l)
       			&& (here->BSIM3geo == group->InstArray[0]->BSIM3geo)
       			&& (here->BSIM3off == group->InstArray[0]->BSIM3off) )
    ...
    printf("BSIM3/SIMD has %d groups and %d instances total\n",ngroups, InstCount);
    if (cp_getvar("no_modsimd", CP_BOOL, NULL, 0))
    {
    	printf("BSIM3 simd disabled at runtime\n");
	BSIM3SIMDinfo.DEVload = BSIM3SIMDload; /* use original load function */
    }
    ...
    CKTcircuit *ckt)
{
    ...
    for (model = (BSIM3model *)inModel; model != NULL;
            model = BSIM3SIMDnextModel(model))
    {
	/* free the group list */
	BSIM3group *group,*ngroup;
