...
typedef int BSIM3instance;
typedef int BSIM3model;
...
int BSIM3_ACM_saturationCurrents
(
	BSIM3model *model,
	BSIM3instance *here,
...
int BSIM3_ACM_junctionCapacitances
(
	BSIM3model *model,
	BSIM3instance *here,
...
ChargeComputationNeeded =
	  ...
	  if(prelim==1)
	  {
	  	here->BSIM3SIMDvbs = vbs;
		here->BSIM3SIMDvgs = vgs;
		here->BSIM3SIMDvds = vds;
		here->BSIM3SIMDqdef = qdef;
		here->BSIM3SIMDcdhat = cdhat;
		here->BSIM3SIMDcbhat = cbhat;
		here->BSIM3SIMDCheck = Check;
		return OK;
	  }
	  else if (prelim==0)
	  {
	  	if(here->BSIM3SIMDCheck==-1)
			printf("error load unitilized data\n");
	  	vbs = here->BSIM3SIMDvbs;
    		vgs = here->BSIM3SIMDvgs;
    		vds = here->BSIM3SIMDvds;
    		qdef = here->BSIM3SIMDqdef;
    		cdhat = here->BSIM3SIMDcdhat;
    		cbhat = here->BSIM3SIMDcbhat;
    		Check = here->BSIM3SIMDCheck;
	  }
	  #endif
    ...
    Check = here->BSIM3SIMDCheck;
	  ...
	  modesym = 2*(BSIM3mode&0x1)-1;
          here->BSIM3mode = modesym; /* restore +/- 1 */
          /* F.B. all subsequents here->BSIM3mode are replaced by local BSIM3mode +1/0 */
...
/* Vth Calculation */
	  ...
	  T2 = pParam->BSIM3dvt2; /* F.B. */
          ...
          Delt_vth = pParam->BSIM3dvt0 * Theta0 * V0; /* F.B. */
...
/* Effective Vgst (Vgsteff) Calculation */
          ...
          #ifdef SIMDIFYCPP
	  /* F.B. Simd version use call exp only once */
	  T0 = VgstNVt;
...
/* Calculate VACLM */
          /* F.B. moved else clause into default values */
	  VACLM = MAX_EXP;
...
/* Calculate VASCBE */
          /* F.B. moved from else clauses to default val*/
	  VASCBE = MAX_EXP;
...
finished:
          ...
          {   /*  charge storage elements
              ...
              double nstate_qbs = *(ckt->CKTstate0 + here->BSIM3qbs); /* F.B. */
	      double nstate_qbd = *(ckt->CKTstate0 + here->BSIM3qbd); /* F.B. */
              ...
              {   if (czbs > 0.0)
                  {   arg = 1.0 - vbs / model->BSIM3PhiB;
                      if (MJ == 0.5)
                          ...
                          sarg = exp(-MJ * log(arg));
                      nstate_qbs = model->BSIM3PhiB * czbs
                                       * (1.0 - arg * sarg) / (1.0 - MJ);
                  ...
                  {   arg = 1.0 - vbs / model->BSIM3PhiBSW;
                      ...
                      nstate_qbs += model->BSIM3PhiBSW * czbssw
                  ...
                  {   arg = 1.0 - vbs / model->BSIM3PhiBSWG;
                      ...
                      nstate_qbs += model->BSIM3PhiBSWG * czbsswg
	      ...
	      /* F.B. moved memory write outside ifs */
	      *(ckt->CKTstate0 + here->BSIM3qbs) = nstate_qbs;
              ...
              {   if (czbd > 0.0)
                  {   arg = 1.0 - vbd / model->BSIM3PhiB;
                      if (MJ == 0.5)
                          ...
                          sarg = exp(-MJ * log(arg));
                      nstate_qbd = model->BSIM3PhiB * czbd
                                       * (1.0 - arg * sarg) / (1.0 - MJ);
                  ...
                  {   arg = 1.0 - vbd / model->BSIM3PhiBSW;
                      if (MJSW == 0.5)
                          ...
                          sarg = exp(-MJSW * log(arg));
                      nstate_qbd += model->BSIM3PhiBSW * czbdsw
                                       * (1.0 - arg * sarg) / (1.0 - MJSW);
                  ...
                  {   arg = 1.0 - vbd / model->BSIM3PhiBSWG;
                      if (MJSWG == 0.5)
                          ...
                          sarg = exp(-MJSWG * log(arg));
                      nstate_qbd += model->BSIM3PhiBSWG * czbdswg
                                       * (1.0 - arg * sarg) / (1.0 - MJSWG);
	      ...
	      /* F.B. moved memory write outside ifs */
	      *(ckt->CKTstate0 + here->BSIM3qbd) = nstate_qbd;
          }
	  ...
	  CoxWL = model->BSIM3cox * pParam->BSIM3weffCV
          ...
          {   if (here->BSIM3nqsMod == 0)
              ...
              {   if (qcheq > 0.0)
                      ...
                      T0 = -here->BSIM3tconst * qdef * ScalingFactor;
                  ggtg = T0 * here->BSIM3cqgb;
		  here->BSIM3gtg = ggtg;
                  ggtd = T0 * here->BSIM3cqdb;
		  here->BSIM3gtd = ggtd;
                  ggts = T0 * here->BSIM3cqsb;
		  here->BSIM3gts = ggts;
                  ggtb = T0 * here->BSIM3cqbb;
		  here->BSIM3gtb = ggtb;
          ...
          {   if (here->BSIM3nqsMod == 0)
              ...
              {   if (qcheq > 0.0)
                  ...
                  ggtg = T0 * here->BSIM3cqgb;
		  here->BSIM3gtg = ggtg;
                  ggts = T0 * here->BSIM3cqdb;
		  here->BSIM3gtd = ggts;
                  ggtd = T0 * here->BSIM3cqsb;
		  here->BSIM3gts = ggtd;
                  ggtb = T0 * here->BSIM3cqbb;
		  here->BSIM3gtb = ggtb;
                  gqdef = ScalingFactor * ag0;
...
line850:
          ...
          ggtg = ggtd = ggtb = ggts = 0.0;
	  /* F.B. moved dxpart assignment outside sxpart rhs */
	  dxpart=0.6;
	  if(BSIM3mode)
	  	dxpart=0.4;
          sxpart = (1.0 - dxpart);
