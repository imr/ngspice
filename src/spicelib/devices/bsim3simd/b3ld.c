...
#ifdef USE_OMP
int BSIM3SIMDLoadOMP(BSIM3instance *here, CKTcircuit *ckt);
void BSIM3SIMDLoadRhsMat(GENmodel *inModel, CKTcircuit *ckt);
#endif
...
#pragma omp parallel for
    for (idx = 0; idx < model->BSIM3InstCount; idx++) {
        ...
        int local_error = BSIM3SIMDLoadOMP(here, ckt);
        if (local_error)
            error = local_error;
    }
...
int BSIM3SIMDLoadOMP(BSIM3instance *here, CKTcircuit *ckt) {
BSIM3model *model = BSIM3SIMDmodPtr(here);
#else
BSIM3model *model = (BSIM3model*)inModel;
BSIM3instance *here;
#endif
