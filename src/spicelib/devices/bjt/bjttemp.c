...
void BJTtempUpdate(BJTmodel *inModel, BJTinstance *here, double Temp, CKTcircuit *ckt)
{
 ...
 */
    ...
    xfc = log(1-model->BJTdepletionCapCoeff);
        ...
        ratlog = log(Temp/model->BJTtnom);
        ratio1 = Temp/model->BJTtnom -1;
...
BJTtemp(GENmodel *inModel, CKTcircuit *ckt)
{
    ...
    for( ; model != NULL; model = BJTnextModel(model)) {
        ...
        for(here=BJTinstances(model);here;here=BJTnextInstance(here)) {
            ...
            if(!here->BJTdtempGiven) here->BJTdtemp = 0.0;

            if(!here->BJTtempGiven)
                here->BJTtemp = ckt->CKTtemp + here->BJTdtemp;

            BJTtempUpdate(model, here, here->BJTtemp, ckt);
